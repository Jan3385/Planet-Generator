namespace GL {
    ...
    /// @param strideOverride Optional override for the stride (0 = use sizeof(T)) - useful for interleaved attributes
    ...
    void VertexArray::AddAttribute(GLuint layoutIndex, GLint size, const Buffer<T, GL::BufferTarget::ArrayBuffer>& buffer, 
        GLboolean normalized, size_t offset, GLuint divisor, size_t strideOverride) const {
        this->Bind();

        buffer.Bind();
        glEnableVertexAttribArray(layoutIndex);

        size_t stride = strideOverride;
        if(strideOverride == 0)
            stride = std::is_class<T>::value ? sizeof(T) : 0;
