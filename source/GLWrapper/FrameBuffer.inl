...
inline FrameBuffer<ColorType, DepthStencilType>::FrameBuffer(uint8_t MSAA_Samples)
{
    ...
    if constexpr (ColorType == FrameBufferType::RenderBuffer) {
        ...
        if(MSAA_Samples > 1)
            glRenderbufferStorageMultisample(GL_RENDERBUFFER, MSAA_Samples, GL_RGB, 800, 600);
        else
        ...
        if(MSAA_Samples > 1)
            glTexImage2DMultisample(GL_TEXTURE_2D_MULTISAMPLE, MSAA_Samples, GL_RGB, 800, 600, GL_TRUE);
        else
        ...
        GLuint textureTarget = (MSAA_Samples > 1) ? GL_TEXTURE_2D_MULTISAMPLE : GL_TEXTURE_2D;
        glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, textureTarget, attachmentColor, 0);
    }
    ...
    if constexpr (DepthStencilType == FrameBufferType::RenderBuffer) {
        ...
        if(MSAA_Samples > 1)
            glRenderbufferStorageMultisample(GL_RENDERBUFFER, MSAA_Samples, GL_DEPTH24_STENCIL8, 800, 600);
        else
        ...
        glBindTexture(GL_TEXTURE_2D, attachmentDepthStencil);

        if(MSAA_Samples > 1)
            glTexImage2DMultisample(GL_TEXTURE_2D_MULTISAMPLE, MSAA_Samples, GL_DEPTH24_STENCIL8, 800, 600, GL_TRUE);
        else
        ...
        GLuint textureTarget = (MSAA_Samples > 1) ? GL_TEXTURE_2D_MULTISAMPLE : GL_TEXTURE_2D;
        glFramebufferTexture2D(GL_FRAMEBUFFER, GL_DEPTH_STENCIL_ATTACHMENT, textureTarget, attachmentDepthStencil, 0);
    }
